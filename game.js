...
const titles = {
  ...
  hangman: function () {
    return gradient("yellow", "orange", "yellow", "orange", "yellow").multiline(`\n${this.hm.join("\n")}\n`);
  },
};
const hangman = {
  ...
  displayTopicsMenu() {
    let heading = [center(`┌┬┐┌─┐┌─┐┬┌─┐┌─┐`, 64), center(` │ │ │├─┘││  └─┐`, 64), center(` ┴ └─┘┴  ┴└─┘└─┘`, 64)];
    let topicsStr = "";
...
// a nice start animation sadly only working in Linux Bash Terminal ;)
...
// }
function start() {
  console.log(gradient("yellow", "orange", "yellow", "orange", "yellow").multiline(`\n${titles.welcome.join("\n")}\n`));
  console.log(gradient("yellow", "orange", "yellow", "orange", "yellow").multiline(`\n${titles.to.join("\n")}\n`));
  console.log(gradient("yellow", "orange", "yellow", "orange", "yellow").multiline(`\n${titles.hm.join("\n")}\n`));
  const timeout = setTimeout(welcomeUser, 1500);
}
